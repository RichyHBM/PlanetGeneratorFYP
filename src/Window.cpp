...
int Window::mWindowCount = 0;
bool Window::mGLFWInitialised = false;

Window::Window()
{
	mWindow = NULL;
	if(Window::mWindowCount == 0
		&& Window::mGLFWInitialised == false)
	{
		if( !glfwInit() )
		{
			return;
		}
		Window::mGLFWInitialised = true;
	}
...
Window::~Window()
{
	...
	if(Window::mWindowCount == 0 && Window::mGLFWInitialised)
	{
		Window::mGLFWInitialised = false;
		glfwTerminate();
	}
...
void Window::SetCursor(CursorState pState)
{
	switch (pState)
	{
	case Hidden:
